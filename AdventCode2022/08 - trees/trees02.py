...
def eliminate_invisibles(theMap):
  ...
  for segment in theMap:    
    view=[0 for i in range(10)]
    print("initial view:",view)
    i=0
    for tree in segment:

      segment[i]=view[tree-1]
      new_view=[1 for i in view[:tree]]
      new_view.extend([i+1 for i in view[tree:]])
      view=list(new_view)
      i+=1
      print("segment, i, view", i, segment, view)
...
col=[[x * y for x,y in zip(a,b)] for a,b in zip(colA,colB)]
print("COLS JOINED")
printMap(col)
rowB=[x[::-1] for x in rowB]
print("ROW A{}\nROW B{}".format(rowA, rowB))
row=[[x * y for x,y in zip(a,b)] for a,b in zip(rowA,rowB)]
print("ROWS JOINED")
...
final=[[x * y for x,y in zip(a,b)] for a,b in zip(row,col_transformed)]
print("FINAL")
...
result = max([tree for segment in final for tree in segment])
print(result)
#os.system("cls")
# print("ZIP TEST")
# print(colB)
